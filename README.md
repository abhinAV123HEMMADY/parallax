...
Parallax is a topic-driven tutoring platform: type a topic (or snap a photo of your worked
...
Two things make Parallax more than an AI-tutor wrapper: every learning signal feeds one
...
- **Exam-date-aware scheduling ("peak on the day").** Standard spaced repetition optimizes
  indefinite retention. Give Parallax an exam date and the FSRS planner keeps the normal
...
served from one shared package (`shared/parallax_embed`) that the backend, the seed script and
...
The placeholder is still reachable via `PARALLAX_EMBED=hash` so the project stays runnable where
...
Parallax already found the exact timestamp in a lecture that explains a topic. Watch & Note turns
...
The extension is a capture client, not a second Parallax. It holds the panel, capture, video Q&A and
...
2. **Error-step localization.** On the Learn page, tap "…or try a sample photo" (or snap a
   real photo of worked math with `ANTHROPIC_API_KEY` set). Parallax transcribes the steps,
...
ordering assertion catches a regression back to it. Running the suite under `PARALLAX_EMBED=hash`
